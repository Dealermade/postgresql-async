...
import com.github.dealermade.async.db.util.FutureUtils._
  ...
  "connection" should {
    ...
    "be able to execute create stored procedure" in {
      withConnection { connection =>
        val future = for (drop <- connection.sendQuery("DROP PROCEDURE IF exists helloWorld;");
    ...
    "be able to call stored procedure" in {
      withConnection { connection =>
        val future = for (drop <- connection.sendQuery("DROP PROCEDURE IF exists constTest;");
                          create <- connection.sendQuery(
                            """
              ...
              """
                          ...
                          arg <- connection.sendQuery("SELECT @arg")) yield arg
    ...
    "be able to call stored procedure with input parameter" in {
      withConnection { connection =>
        val future = for (drop <- connection.sendQuery("DROP PROCEDURE IF exists addTest;");
                          create <- connection.sendQuery(
                            """
              ...
              """
                          ...
                          res <- connection.sendQuery("SELECT @sm")) yield res
        val result: Option[ResultSet] = awaitFuture(future).rows
        result.isDefined === true
        val rows = result.get
        rows.size === 1
    ...
    "be able to remove stored procedure" in {
      withConnection { connection =>
        val createResult: Option[ResultSet] = awaitFuture(
          for (drop <- connection.sendQuery("DROP PROCEDURE IF exists remTest;");
               create <- connection.sendQuery(
                 """
        ...
        val removeResult: Option[ResultSet] = awaitFuture(
          for (drop <- connection.sendQuery("DROP PROCEDURE remTest;");
               routine <- connection.sendQuery(
                 """
                  SELECT routine_name FROM INFORMATION_SCHEMA.ROUTINES WHERE routine_name="remTest"
                """
