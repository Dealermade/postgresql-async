...
import com.github.dealermade.async.db.util.FutureUtils.awaitFuture
    ...
    "runs commands for a transaction in a single connection" in {
      ...
      withPool { pool =>
        val operations = pool.inTransaction { connection =>
          connection.sendPreparedStatement(TransactionInsert, List(id)).flatMap { result =>
            connection.sendPreparedStatement(TransactionInsert, List(id)).map { failure =>
              List(result, failure)
            }
          }
        }
