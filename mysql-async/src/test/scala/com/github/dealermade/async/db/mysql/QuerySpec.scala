...
import com.github.dealermade.async.db.{QueryResult, ResultSet}
...
class QuerySpec extends Specification with ConnectionHelper {
  ...
  "connection" should {
    ...
    "fail if number of args required is different than the number of provided parameters" in {
      ...
      withConnection { connection =>
        executePreparedStatement(connection, "select * from some_table where c = ? and b = ?", "one", "two", "three") must throwAn[
