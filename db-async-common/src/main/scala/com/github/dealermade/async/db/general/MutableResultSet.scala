...
package com.github.dealermade.async.db.general

import collection.mutable.ArrayBuffer
import com.github.dealermade.async.db.{RowData, ResultSet}
...
class MutableResultSet[T <: ColumnData](val columnTypes: IndexedSeq[T]) extends ResultSet {
  ...
  private val columnMapping: Map[String, Int] = this.columnTypes.indices.map(index => (this.columnTypes(index).name, index)).toMap
